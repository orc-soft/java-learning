...
public class MortgageCalc {
    ...
    private static void calculateMortgage(int principal, float air, byte period) {
        double mortgageFormulaFirst = Math.pow(1 + getMonthlyInterestRate(air), getNumberOfPayments(period));
        double mortgage = principal * ((getMonthlyInterestRate(air) * mortgageFormulaFirst) / (mortgageFormulaFirst - 1));
    ...
    private static void paymentSchedule(int principal, float air, byte period) {
        ...
        for (int i = 1; i <= getNumberOfPayments(period); ++i) {
            balance = principal * ((mortgageFormulaFirst) - (Math.pow(1 + getMonthlyInterestRate(air), i))) / (mortgageFormulaFirst - 1);
